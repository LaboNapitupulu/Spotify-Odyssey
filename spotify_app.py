    ...
    fig.update_layout(
        ...
        polar=dict(
            ...
            angularaxis=dict(
                ...
                ticktext=['0', '6', '12', '18'], direction="clockwise",
                rotation=90, gridcolor='#1e293b', linecolor='#1e293b'
            ),
...
cont_now_playing = st.container()
cont_kpi = st.container()
cont_clocks = st.container()
cont_trends = st.container()
cont_hof = st.container()
cont_recent = st.container()

# A. EKSEKUSI JAVASCRIPT REAL-TIME DI AWAL 
html_now_playing = f"""
<!DOCTYPE html>
<html>
<head>
...
<body>
    ...
    <script>
        const token = "{ACCESS_TOKEN}"; const container = document.getElementById('np-container'); 
        let currentSongId = sessionStorage.getItem('np_currentSongId') || "";

        // Pemulihan Cache Instan Anti-Flicker
        const cachedHTML = sessionStorage.getItem('np_HTML');
                ...
                if (res.status === 200) {{
                    ...
                    if (data.is_playing && data.item) {{
                        if (currentSongId !== data.item.id) {{
                            ...
                            sessionStorage.setItem('np_currentSongId', currentSongId);
                            const newHtml = `<div class="now-playing-bar fade-in"><div class="np-details"><img src="${{data.item.album.images[0].url}}" style="width: 60px; height: 60px; min-width: 60px; border-radius: 8px; box-shadow: 0 0 15px #1DB954; object-fit: cover;"><div class="np-text"><p><span class="live-dot"></span>CURRENTLY PLAYING</p><h4>${{data.item.name}}</h4><p style="color:#94a3b8">${{data.item.artists[0].name}}</p></div></div><div style="color:#1DB954; font-weight:bold; font-size:24px;">🎵</div></div>`;
                            container.innerHTML = newHtml;
                            sessionStorage.setItem('np_HTML', newHtml);
                        }} return;
                    }}
                }}
                if (currentSongId !== "sleep") {{
                    currentSongId = "sleep";
                    sessionStorage.setItem('np_currentSongId', "sleep");
                    const newHtml = `<div class="now-playing-bar fade-in" style="border-bottom: 2px solid #334155;"><div class="np-details"><div style="width: 60px; height: 60px; border-radius: 8px; background-color: #1e293b; display: flex; align-items: center; justify-content: center; font-size: 20px;">💤</div><div class="np-text"><p style="color:#475569">STATUS LIVE</p><h4 style="color:#64748b">Tidur...</h4><p style="color:#475569">Tidak ada lagu yang sedang diputar.</p></div></div></div>`;
                    container.innerHTML = newHtml;
                    sessionStorage.setItem('np_HTML', newHtml);
                }}
            }} catch(e) {{}}
...
<body>
    ...
    <script>
        const token = "{ACCESS_TOKEN}"; const tbody = document.getElementById('recent-tbody'); 
        let lastFirstTimestamp = sessionStorage.getItem('rp_lastTimestamp') || "";

        // Pemulihan Cache Instan Anti-Flicker
        const cachedRecent = sessionStorage.getItem('rp_HTML');
        if (cachedRecent) {{ tbody.innerHTML = cachedRecent; }}
                ...
                if (res.status === 200) {{
                    ...
                    if (data.items && data.items.length > 0) {{
                        ...
                        if (lastFirstTimestamp !== latestTimestamp) {{
                            ...
                            sessionStorage.setItem('rp_lastTimestamp', latestTimestamp);
                            let htmlRows = "";
                            ...
                            sessionStorage.setItem('rp_HTML', htmlRows);
                        }}
                    }}
                }}
            }} catch(e) {{}}
...
# Menyisipkan komponen secara instan ke dalam antrean yang sudah dipesan
with cont_now_playing:
